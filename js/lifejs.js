var LifeGame = {
  ...
  */
  ...
  {
    $('#tbStart')[0].className = this.timerId ? 'active' : '';
//    $('#tbRecord')[0].className = this.recordOn ? 'active' : '';
//    $('#tbGetImage')[0].className = this.recordSteps.length ? '' : 'disabled';
  },
  ...
  */
  ...
  step: function()
  {
    ...
    if(!this.draw())
    {
      this.recordOn = false;
      this.stop();
      alert('Игра завершена на ' + this.stepNo + ' шагу');
      return;
    }
    if(this.lastCode == curCode)
    {
      this.stop();
      alert('Появилась статическая колония');
    }
  ...
  zoom: function(dir)
  {
    this.checkSize = (dir > 0) ? Math.min(16, Math.round(this.checkSize * 1.6))
      : Math.max(3, Math.round(this.checkSize / 1.6));
    this.draw();
  },
...
//    $('#tbRecord').click(function() { LifeGame.record(); return false; });
//    $('#tbGetImage').click(function() { LifeGame.getRecordedImage(); return false; });
